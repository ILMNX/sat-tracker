...
export default function SatellitePositions() {
  ...
  const latest = positions[currentIndex] || null;

  // Fetch batched positions once every 60 sec
  useEffect(() => {
    const fetchPositions = async () => {
      try {
        ...
        const lng = -76.014;
        const alt = 0;
        const seconds = 60; // batch 60 sec of positions
    ...
    fetchPositions();
    const interval = setInterval(fetchPositions, 60000); // fetch once per minute
    return () => clearInterval(interval);
  }, []);

  // Animate locally, step through positions every second
  useEffect(() => {
    ...
    const stepper = setInterval(() => {
      setCurrentIndex((prev) => {
        ...
        return prev; // stay at last until new fetch
      });
    }, 1000);
  ...
  // Check if map is centered on latest
  useEffect(() => {
    ...
    <div className="relative max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl p-8">
      ...
      <div className="mb-8 flex gap-8 flex-wrap">
        <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-6 shadow flex-1 min-w-[220px] border border-blue-100">
          ...
          <div className="space-y-2 text-lg">
            <div><span className="font-semibold">Latitude:</span> <span className="text-blue-700">{latest.satlatitude.toFixed(2)}</span></div>
            <div><span className="font-semibold">Longitude:</span> <span className="text-blue-700">{latest.satlongitude.toFixed(2)}</span></div>
            <div><span className="font-semibold">Altitude:</span> <span className="text-blue-700">{latest.sataltitude.toFixed(2)} km</span></div>
            <div><span className="font-semibold">Location:</span> <span className="text-blue-700">{location}</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl overflow-hidden shadow-lg border border-blue-100">
        ...
        >
          ...
          />
          <Marker position={[latest.satlatitude, latest.satlongitude]} icon={satelliteIcon}>
            <Popup>
              <div>
                <p><strong>Latitude:</strong> {latest.satlatitude.toFixed(2)}</p>
                <p><strong>Longitude:</strong> {latest.satlongitude.toFixed(2)}</p>
                <p><strong>Altitude:</strong> {latest.sataltitude.toFixed(2)} km</p>
              </div>
            </Popup>
          </Marker>
        </MapContainer>
      ...
      {/* Log Drawer */}
      <div
        className={`fixed top-0 right-0 h-full w-80 bg-white shadow-2xl border-l z-50 transition-transform duration-300 ${showLog ? "translate-x-0" : "translate-x-full"}`}
        style={{ maxHeight: "100vh", overflowY: "auto" }}
      >
        <div className="flex items-center justify-between p-4 border-b">
          <h3 className="text-lg font-bold text-gray-700">Live Position Log</h3>
          <button onClick={() => setShowLog(false)} className="text-gray-500 hover:text-gray-700" aria-label="Close">✕</button>
        </div>
        <ul className="p-4 space-y-3">
          {positions.map((pos, idx) => (
            <li key={idx} className={`rounded-lg p-3 bg-gray-50 shadow-sm border ${idx === currentIndex ? "border-blue-500 bg-blue-50" : ""}`}>
              <div><span className="font-medium">Lat:</span> {pos.satlatitude.toFixed(2)}</div>
              <div><span className="font-medium">Lng:</span> {pos.satlongitude.toFixed(2)}</div>
              <div><span className="font-medium">Alt:</span> {pos.sataltitude.toFixed(2)} km</div>
            </li>
          ))}
        </ul>
      </div>
